...
class StateTimeSelection:
    """One snapshot per state epoch, finalized when the publication stream advances."""
    def __init__(self, frame_times, audit):
        if len({round(t,9) for t in frame_times.values()}) != len(frame_times):
            raise ValueError('ambiguous duplicate scoring epochs')
    ...
    def choose(self, message, received):
        source = message.source_header.stamp.to_sec()
        state = message.header.stamp.to_sec()
        row = dict(order=len(self.rows), source_stamp=source, state_stamp=state,
                   received_stamp=received, score_stamp=None, decision='')
        self.rows.append(row)
        def reject(reason):
            row['decision'] = reason
            self.audit[reason] = self.audit.get(reason, 0)+1
            return None
        if not all(math.isfinite(t) and t > 0 for t in (source, state, received)):
            return reject('invalid_publication_timestamp')
        if received < self.last_received-1.e-9:
            return reject('nonmonotonic_availability')
        self.last_received=received
        if source > state+1.e-9:
            return reject('source_after_state')
        # last_prediction is the state epoch, even for a corrected posterior.
        for track in message.tracks:
            prediction = track.last_prediction.to_sec()
            correction = track.last_correction.to_sec()
            detection = track.last_detection.to_sec()
            if (not all(math.isfinite(t) for t in (prediction, correction, detection)) or
                    abs(prediction-state)>1.e-9 or correction<0 or detection<0 or
                    max(correction,detection)>state+1.e-9):
                return reject('inconsistent_state_or_observation_time')
        # /clock delivery and rosbag receipt timestamps can lag acquisition
        # time. Availability is the publication event order, not this clock
        # subtraction; retain discrepancies as audit evidence, not frame loss.
        if state > received+1.e-9:
            self.audit['receipt_clock_before_state'] = self.audit.get('receipt_clock_before_state',0)+1
        if state < self.watermark-1.e-9:
            return reject('late_past_state_rejected')
        key = round(state,9)
        score = self.frame_times.get(key)
        if score is not None and score > received+1.e-9:
            self.audit['receipt_clock_before_score'] = self.audit.get('receipt_clock_before_score',0)+1
        self.watermark = max(self.watermark,state)
        if score is None:
            return reject('state_outside_scoring_grid')
        row['score_stamp'] = score
        if key in self.selected:
            self.selected[key]['decision'] = 'superseded_same_state'
            self.audit['same_state_replacements'] = self.audit.get('same_state_replacements',0)+1
        if abs(source-state)>1.e-9:
            self.audit['state_time_remapped_publications'] = self.audit.get('state_time_remapped_publications',0)+1
...
def load_run(path, algorithm, score_start, score_end, frame_times=None, time_audit=None,
    ...
    selection = None if aerocover or frame_times is None else StateTimeSelection(frame_times,time_audit)
    with rosbag.Bag(path) as bag:
        for topic, message, received_time in bag.read_messages(topics=topics):
            ...
            if topic == completion_topic:
                ...
                received = received_time.to_sec() if received_time is not None else None
                if aerocover:
                    ...
                    if any(max(track.last_prediction.to_sec(), track.last_correction.to_sec())
                           > stamp+1.e-9 for track in message.tracks):
                        time_audit['future_state_publications_rejected'] = time_audit.get(
                            'future_state_publications_rejected', 0)+1
                        continue
                    if abs(message.header.stamp.to_sec()-stamp) > 1.e-9:
                        raise RuntimeError('AeroCOVER Tracks.header is not its decision epoch')
                else:
                    if received is None:
                        raise RuntimeError('VoFOD state selection requires recorded message availability time')
                    stamp = selection.choose(message,received)
                    if stamp is None:
                        continue
                predictions = []
                frame_tracks = []
    ...
    if selection is not None:
        if publication_audit is not None:
            publication_audit.extend(selection.rows)
        time_audit['selected_state_frames'] = len(selection.selected)
        offsets=[row['received_stamp']-row['score_stamp'] for row in selection.selected.values()]
        time_audit['selected_receipt_clock_offset_s'] = distribution(offsets)
        time_audit['selected_receipt_clock_offset_s']['min'] = min(offsets) if offsets else None
        time_audit['selection_protocol'] = 'publication-event order, state-keyed last legal snapshot before newer state; receipt clock is audit only; no retrospective refill'
...
def evaluate(source_bag, run_bag, algorithm, scenario_file, output_dir,
    ...
    (track_frames, all_tracks, background, free, timing,
     diagnostics) = load_run(run_bag, algorithm, score_start, score_end,
                             frame_times, time_audit, publication_audit)
    if algorithm.startswith("AeroCOVER"):
    ...
    if not algorithm.startswith('AeroCOVER'):
        with open(os.path.join(output_dir,'publication_selection.csv'),'w',newline='') as stream:
            writer=csv.DictWriter(stream,fieldnames=['order','source_stamp','state_stamp','received_stamp','score_stamp','decision'])
            writer.writeheader();writer.writerows(publication_audit)
    rows = []
    ...
    with open(os.path.join(output_dir, "track_timeseries.csv"), "w", newline="",
        ...
        columns = (
            "stamp", "source_stamp", "state_stamp", "received_stamp", "id", "state", "existence", "stale_s",
