#!/usr/bin/env python3
"""OPEN Mid360: ground centered in layer two, ready ratio unchanged."""
import csv
import json
...
REPORT = ['# OPEN/VoFOD-Mid360：地面第二层中央对照', '',
...
def main():
    ...
    for scene, sensor, ratio in CASES:
        ...
        expected['operation_area']['center']['z'] = 8.8125
        expected['operation_area']['size']['z'] = 18.375
        ...
        assert lower == -.375 and lower + (1+.5)*.25 == 0
        assert yaml.safe_load(config.read_text()) == expected, config
        print('PASS only map z changed; ground centered in layer two:', config, flush=True)
        if validate_only:
            continue
        ...
        for d, value in [(BASE/'runs'/scene/method, -.625), (directory, -.375)]:
