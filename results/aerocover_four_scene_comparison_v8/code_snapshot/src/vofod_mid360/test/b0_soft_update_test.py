...
class B0SoftUpdateContract(unittest.TestCase):
    ...
    def _assert_map_revision_evidence(self, value, diagnostics):
        ...
        self.assertEqual(value.header.stamp, diagnostics.header.stamp)
        self.assertEqual(value.header.frame_id, diagnostics.header.frame_id)
        ...
        self.assertGreater(value.map_revision, diagnostics.map_revision)
        self.assertEqual(
            value.voxel_count,
        ...
        self.assertGreater(value.free_updated_this_revision_count, 0)
    ...
    def _assert_diagnostics(self, value, previous_revision):
        ...
        self.assertEqual(value.background_mode, "native_rangefinder")
        self.assertEqual(value.map_execution_mode, "upstream_native")
        ...
        # These three are produced by the asynchronous native ray worker and
        # appear on the next diagnostic that observes its completion.
        self.assertIn(value.rejected_transform, (0, 1))
        self.assertIn(value.rejected_geometry, (0, 1))
        self.assertIn(value.rejected_nonpositive_endpoint, (0, 1))
        self.assertEqual(value.start_outside_map, 0)
        self.assertTrue(value.first_input_ack)
        self.assertEqual(value.initialization_window_scans, 0)
        self.assertFalse(value.initialization_ready)
        self.assertEqual(value.certified_background_cells, 0)
        ...
        self.assertTrue(value.ray_task_submitted or value.ray_task_skipped_busy)
        for duration in (
    ...
    def _assert_graph_isolation(self):
        ...
        self.assertEqual(
            algorithm_inputs,
            {self.points_topic, self.rays_topic, "/tf", "/tf_static",
    ...
    def test_soft_update_contract(self):
        ...
        # Native VoFOD computes scan 100 rays asynchronously and applies them
        # only after a later point update becomes visible.
        self._publish_scan(101)
        ...
        # A short Mid-360 endpoint remains UNKNOWN during shared cold-start.
        ...
        second_diag = self._publish_scan(102)
        self._assert_diagnostics(second_diag, first_revision)
        self._publish_scan(103)
        self._assert_map_revision_evidence(
            self._wait_for_map_revision_evidence(102), second_diag
        )
