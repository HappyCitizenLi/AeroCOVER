...
namespace vofod
{
  ...
  private:
    struct AsyncRaySummary
    {
      uint32_t scan_id = 0U;
      bool timed_out = false;
      RayUpdateStats stats;
      float accumulation_ms = 0.0F;
      float wait_ms = 0.0F;
      float apply_ms = 0.0F;
    };
    ...
    {
      SeparateBackgroundCleanupStats stats;
    ...
    void initialize()
    {
      ...
      requireParam("separate_background/max_distance",
                   &baseline_config_.separated_background_distance_m);
      requireParam("separate_background/period_s",
                   &separate_background_period_s_);
      int separated_background_minimum_sure_voxels = 0;
      requireParam("separate_background/min_sure_voxels",
                   &separated_background_minimum_sure_voxels);
      if (separated_background_minimum_sure_voxels <= 0)
        throw std::runtime_error("invalid separate-background configuration");
      baseline_config_.separated_background_minimum_sure_voxels =
          static_cast<size_t>(separated_background_minimum_sure_voxels);
      requireParam("input/geometry_consistency_tolerance_m",
                   &geometry_consistency_tolerance_m_);
      requireParam("background/mode", &background_mode_);
      requireParam("background/sufficient_points_ratio",
                   &background_sufficient_points_ratio_);
      if (background_mode_ != "native_rangefinder")
        throw std::runtime_error(
            "background/mode must be native_rangefinder");
      ...
      if (background_mode_ == "native_rangefinder")
      {
        ...
        range_subscriber_ = private_nh_.subscribe(
            "height_rangefinder", 100, &VoFOD::rangeCallback, this);
      }
      cleanup_timer_ = private_nh_.createTimer(
          ros::Duration(separate_background_period_s_),
          &VoFOD::cleanupTimer, this);
    ...
    void validateParameters() const
    {
      ...
      if (!std::isfinite(geometry_consistency_tolerance_m_) ||
          geometry_consistency_tolerance_m_ < 0.0)
        throw std::runtime_error(
            "input geometry tolerance is invalid");
      ...
      if (!std::isfinite(background_sufficient_points_ratio_) ||
          background_sufficient_points_ratio_ <= 0.0 ||
          background_sufficient_points_ratio_ > 1.0)
        throw std::runtime_error("background sufficient-points ratio is invalid");
      if (!std::isfinite(separate_background_period_s_) ||
          separate_background_period_s_ <= 0.0)
        throw std::runtime_error("separate-background period is invalid");
    ...
    void loadBlockedPatternIndices()
    {
      ...
      if (ranges.getType() != XmlRpc::XmlRpcValue::TypeArray)
        throw std::runtime_error(
            "body_mask/blocked_pattern_ranges must be an array");
      for (int index = 0; index < ranges.size(); ++index)
      {
        if (ranges[index].getType() != XmlRpc::XmlRpcValue::TypeArray ||
            ranges[index].size() != 2 ||
            ranges[index][0].getType() != XmlRpc::XmlRpcValue::TypeInt ||
            ranges[index][1].getType() != XmlRpc::XmlRpcValue::TypeInt)
          throw std::runtime_error(
              "body-mask pattern ranges must be [first, last] integer pairs");
        const int first = static_cast<int>(ranges[index][0]);
        const int last = static_cast<int>(ranges[index][1]);
        if (first < 0 || last < first)
          throw std::runtime_error("body-mask pattern range is invalid");
    ...
    void initializeMap()
    {
      ...
      point_protected_.assign(voxel_map_.size(), 0U);
      last_revision_free_update_count_ = 0U;
      map_revision_ = 0U;
      detection_id_ = 0U;
      detection_iterations_ = 0U;
      background_seed_effective_ = false;
      background_points_sufficient_ = false;
      sure_background_sufficient_ = false;
      detection_ready_ = false;
      first_seed_effective_stamp_ = 0.0;
      detection_ready_stamp_ = 0.0;
      last_ray_summary_ = AsyncRaySummary();
      last_cleanup_summary_ = CleanupSummary();
    ...
    void updateBackgroundReadiness(const ros::Time& stamp)
    {
      const VoxelMap::vec3i_t sizes = voxel_map_.sizes();
      const double required = static_cast<double>(sizes.x()) * sizes.y() *
          background_sufficient_points_ratio_;
      background_points_sufficient_ =
          static_cast<double>(voxel_map_.nVoxelsOver(new_obstacle_threshold_)) >
          required;
      const bool ready =
          background_points_sufficient_ && sure_background_sufficient_;
      if (ready && !detection_ready_)
        detection_ready_stamp_ = stamp.toSec();
      detection_ready_ = ready;
    }

    void rangeCallback(const sensor_msgs::RangeConstPtr& message)
    {
      if (message->header.stamp.isZero() || !std::isfinite(message->range) ||
          message->range <= message->min_range ||
          message->range >= message->max_range ||
          message->header.frame_id.empty())
        return;
      geometry_msgs::PointStamped local;
      local.header = message->header;
      local.point.x = message->range;
      geometry_msgs::PointStamped world;
      try
      {
        world = tf_buffer_.transform(local, world_frame_id_, ros::Duration(0.2));
      }
      catch (const tf2::TransformException& error)
      {
        NODELET_WARN_STREAM_THROTTLE(
            1.0, "[VoFOD-Mid360] rangefinder transform failed: " << error.what());
        return;
      }
      std::unique_lock<std::mutex> lock(map_mutex_);
      if (!voxel_map_.inLimits(world.point.x, world.point.y, world.point.z))
        return;
      size_t linear = 0U;
      if (!voxel_map_.tryLinearIndex(
              voxel_map_.coordToIdx(VoxelMap::vec3_t(
                  world.point.x, world.point.y, world.point.z)), &linear))
        return;
      float& value = voxel_map_.atLinear(linear);
      if (!std::isfinite(value))
        return;
      value = 0.5f * (value + point_score_);
      ++update_counts_[linear];
      update_classes_[linear] = BACKGROUND_SEED;
      last_update_revisions_[linear] = map_revision_;
      if (!background_seed_effective_)
      {
        background_seed_effective_ = true;
        first_seed_effective_stamp_ = message->header.stamp.toSec();
      }
      updateBackgroundReadiness(message->header.stamp);
    }

    void rayTask(std_msgs::Header header, const uint32_t scan_id,
                 VoxelMap geometry, std::vector<RayUpdateRay> rays,
                 const uint64_t start_detection_iterations)
    {
      AsyncRaySummary summary;
      summary.scan_id = scan_id;
      try
      {
        const Clock::time_point accumulation_start = Clock::now();
        RayAccumulation accumulation = ray_core_->accumulate(geometry, rays);
        summary.accumulation_ms = elapsedMs(accumulation_start);

        const Clock::time_point wait_start = Clock::now();
        std::unique_lock<std::mutex> lock(map_mutex_);
        const bool ready = detection_cv_.wait_for(
            lock, std::chrono::milliseconds(800),
            [this, start_detection_iterations]
            {
              return shutting_down_.load() ||
                  detection_iterations_ > start_detection_iterations;
            });
        summary.wait_ms = elapsedMs(wait_start);
        if (shutting_down_.load())
        {
          ray_running_.store(false);
          return;
        }
        if (!ready)
        {
          summary.timed_out = true;
          last_ray_summary_ = summary;
          ray_running_.store(false);
          return;
        }
        const double repetitions = static_cast<double>(
            detection_iterations_ - start_detection_iterations);
        const Clock::time_point apply_start = Clock::now();
        summary.stats = ray_core_->apply(
            voxel_map_, accumulation,
            [this](const size_t index)
            {
              return point_protected_.at(index) != 0U;
            }, repetitions);
        summary.apply_ms = elapsedMs(apply_start);

        if (map_revision_ == std::numeric_limits<uint64_t>::max())
          throw std::overflow_error("map revision exhausted in ray task");
        const uint64_t revision = ++map_revision_;
        uint64_t free_updated = 0U;
        for (const size_t linear : accumulation.touched_indices)
        {
          if (point_protected_.at(linear) != 0U ||
              !std::isfinite(voxel_map_.atLinear(linear)))
            continue;
          ++free_updated;
          const auto& exposure = accumulation.per_voxel.at(linear);
          ++update_counts_[linear];
          last_update_revisions_[linear] = revision;
          last_free_update_revisions_[linear] = revision;
          if (exposure.valid_return_length_m > 0.0 &&
              exposure.no_return_length_m > 0.0)
            update_classes_[linear] = FREE_MIXED;
          else if (exposure.valid_return_length_m > 0.0)
            update_classes_[linear] = FREE_VALID;
          else
            update_classes_[linear] = FREE_NO_RETURN;
        }
        if (free_updated != summary.stats.voxels_updated)
          throw std::logic_error("async free-update accounting mismatch");
        std::fill(point_protected_.begin(), point_protected_.end(), 0U);
        last_revision_free_update_count_ = free_updated;
        last_ray_summary_ = summary;
        map_revision_evidence_pub_.publish(
            makeMapRevisionEvidence(header, scan_id, revision));
      }
      catch (const std::exception& error)
      {
        NODELET_ERROR_STREAM("[VoFOD-Mid360] native ray task failed: "
                             << error.what());
    ...
    void submitRayTask(const std_msgs::Header& header, const uint32_t scan_id,
                       std::vector<RayUpdateRay> rays,
                       vofod_mid360::MapUpdateDiagnostics* diagnostics)
    {
      if (ray_running_.exchange(true))
      {
        diagnostics->ray_task_skipped_busy = true;
        return;
      }
      if (ray_thread_.joinable())
        ray_thread_.join();
      VoxelMap geometry;
      geometry.resizeAs(voxel_map_);
      diagnostics->ray_task_submitted = true;
      ray_thread_ = std::thread(
          &VoFOD::rayTask, this, header, scan_id, std::move(geometry),
          std::move(rays), detection_iterations_);
    }

    void cleanupTimer(const ros::TimerEvent&)
    {
      if (shutting_down_.load())
        return;
      VoxelMap snapshot;
      uint64_t start_detection_iterations = 0U;
      {
        std::lock_guard<std::mutex> lock(map_mutex_);
        snapshot = voxel_map_;
        start_detection_iterations = detection_iterations_;
      }
      const Clock::time_point start = Clock::now();
      SeparateBackgroundCleanupStats cleanup =
          baseline_core_->cleanupSeparatedBackground(&snapshot);
      CleanupSummary summary;
      summary.stats = cleanup;
      summary.elapsed_ms = elapsedMs(start);

      std::lock_guard<std::mutex> lock(map_mutex_);
      const double repetitions = static_cast<double>(std::max<uint64_t>(
          1U, detection_iterations_ - start_detection_iterations));
      const float retain = static_cast<float>(std::pow(0.5, repetitions));
      uint64_t revision = map_revision_;
      if (!cleanup.updated_voxels.empty())
      {
        if (map_revision_ == std::numeric_limits<uint64_t>::max())
          return;
      ...
      for (const size_t linear : cleanup.updated_voxels)
      {
        float& value = voxel_map_.atLinear(linear);
        value = retain * value + (1.0f - retain) * ray_score_;
        ++update_counts_.at(linear);
        update_classes_.at(linear) = SEPARATED_BACKGROUND_CLEAR;
        last_update_revisions_.at(linear) = revision;
      }
      sure_background_sufficient_ = cleanup.sure_cluster_count > 0U;
      last_cleanup_summary_ = std::move(summary);
      updateBackgroundReadiness(ros::Time::now());
    }
        ...
        const mid360_ray_msgs::CheckedRayBundleConstPtr& rays)
    {
      ...
      diagnostics.background_mode = background_mode_;
      diagnostics.map_execution_mode = "upstream_native";
      ...
      updateBackgroundReadiness(rays->header.stamp);
      baseline_core_->partitionCloseFar(
          voxel_map_, *weighted_cloud, &clusters);
      applyPointEvidence(
          *weighted_cloud, clusters, next_map_revision, &point_protected_,
          &diagnostics);
      ++detection_iterations_;
      map_revision_ = next_map_revision;
      last_revision_free_update_count_ = 0U;
      detection_cv_.notify_all();
      submitRayTask(rays->header, rays->scan_id, std::move(update_rays),
                    &diagnostics);
      ...
      const bool initialization_ready = detection_ready_;
      if (initialization_ready)
        baseline_core_->classifyFar(
            &voxel_map_, *weighted_cloud, observer, &clusters);
      diagnostics.initialization_window_scans = 0U;
      diagnostics.initialization_ready = initialization_ready;
      diagnostics.certified_background_cells = 0U;
      ...
      diagnostics.last_completed_ray_scan_id = last_ray_summary_.scan_id;
      diagnostics.ray_task_timed_out = last_ray_summary_.timed_out;
      diagnostics.last_completed_ray_voxels = checkedCast(
          last_ray_summary_.stats.voxels_updated);
      diagnostics.ray_accumulation_ms = last_ray_summary_.accumulation_ms;
      diagnostics.ray_wait_ms = last_ray_summary_.wait_ms;
      diagnostics.ray_apply_ms = last_ray_summary_.apply_ms;
      diagnostics.accepted_valid_return = checkedCast(
          last_ray_summary_.stats.accepted_valid_return);
      diagnostics.accepted_no_return = checkedCast(
          last_ray_summary_.stats.accepted_no_return);
      diagnostics.rejected_transform += checkedCast(
          last_ray_summary_.stats.skipped_transform);
      diagnostics.rejected_geometry += checkedCast(
          last_ray_summary_.stats.skipped_geometry);
      diagnostics.rejected_nonpositive_endpoint += checkedCast(
          last_ray_summary_.stats.skipped_nonpositive_endpoint);
      diagnostics.start_outside_map = checkedCast(
          last_ray_summary_.stats.start_outside_map);
      diagnostics.positive_ray_segments = checkedCast(
          last_ray_summary_.stats.positive_segments);
      diagnostics.touched_voxels = checkedCast(
          last_ray_summary_.stats.unique_voxels_touched);
      diagnostics.protected_voxels = checkedCast(
          last_ray_summary_.stats.voxels_protected);
      diagnostics.free_updated_voxels = checkedCast(
          last_ray_summary_.stats.voxels_updated);
      diagnostics.valid_return_path_m =
          last_ray_summary_.stats.valid_return_path_m;
      diagnostics.no_return_path_m =
          last_ray_summary_.stats.no_return_path_m;

      diagnostics.separate_background_clusters = checkedCast(
          last_cleanup_summary_.stats.cluster_count);
      diagnostics.separate_background_sure_clusters = checkedCast(
          last_cleanup_summary_.stats.sure_cluster_count);
      diagnostics.separate_background_updated_voxels = checkedCast(
          last_cleanup_summary_.stats.updated_voxels.size());
      diagnostics.separate_background_ready = sure_background_sufficient_;
      diagnostics.separate_background_ms = last_cleanup_summary_.elapsed_ms;
      diagnostics.background_seed_effective = background_seed_effective_;
      diagnostics.detection_ready = detection_ready_;
      diagnostics.first_seed_effective_stamp = first_seed_effective_stamp_;
      diagnostics.detection_ready_stamp = detection_ready_stamp_;
      ...
      diagnostics.map_revision = map_revision_;
      diagnostics.detection_iteration = detection_iterations_;
        ...
        const vofod_mid360::MapUpdateDiagnostics& diagnostics)
    {
      ...
      status.detection_active = detection_ready_;
  ...
  private:
    ...
    std::thread ray_thread_;
    std::atomic<bool> ray_running_{false};
    ...
    std::vector<uint8_t> point_protected_;
    AsyncRaySummary last_ray_summary_;
    CleanupSummary last_cleanup_summary_;
    ...
    double background_sufficient_points_ratio_ = 0.15;
    double separate_background_period_s_ = 0.1;
    uint64_t detection_iterations_ = 0U;
    bool background_seed_effective_ = false;
    bool background_points_sufficient_ = false;
    bool sure_background_sufficient_ = false;
    bool detection_ready_ = false;
    double first_seed_effective_stamp_ = 0.0;
    double detection_ready_stamp_ = 0.0;
