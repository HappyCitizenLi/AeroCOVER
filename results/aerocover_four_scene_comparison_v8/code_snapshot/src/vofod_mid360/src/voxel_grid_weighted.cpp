...
namespace vofod
{
  namespace
  {
    ...
    struct WeightedPointIndex
    {
      unsigned int idx;
      unsigned int cloud_point_index;
      int ijk0, ijk1, ijk2;

      WeightedPointIndex(unsigned int idx_, unsigned int cloud_point_index_, int ijk0_, int ijk1_, int ijk2_)
        : idx(idx_), cloud_point_index(cloud_point_index_), ijk0(ijk0_), ijk1(ijk1_), ijk2(ijk2_)
      {}

      bool operator<(const WeightedPointIndex& other) const
      {
  ...
  void VoxelGridWeighted::filterImpl(PointCloudOut& output)
  {
    ...
    std::vector<WeightedPointIndex> index_vector;
    ...
    for (std::vector<int>::const_iterator it = indices_->begin (); it != indices_->end (); ++it)
    {
      ...
      const WeightedPointIndex cpidx(static_cast<unsigned int> (idx), *it, ijk0, ijk1, ijk2);
      index_vector.push_back(cpidx);
    }

    // Second pass: sort the index_vector vector using value representing target cell as index
    // in effect all points belonging to the same output cell will be next to each other
    std::sort (index_vector.begin (), index_vector.end (), std::less<WeightedPointIndex>());
    ...
    for (unsigned int cp = 0; cp < first_and_last_indices_vector.size (); ++cp)
    {
      ...
      const std::uint32_t weight = static_cast<std::uint32_t>(last_index - first_index);
