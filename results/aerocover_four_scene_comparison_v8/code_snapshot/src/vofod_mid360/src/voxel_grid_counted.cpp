...
namespace vofod
{
  namespace
  {
    ...
    struct CountedPointIndex
    {
      unsigned int idx;
      unsigned int cloud_point_index;
      int ijk0, ijk1, ijk2;

      CountedPointIndex(unsigned int idx_, unsigned int cloud_point_index_, int ijk0_, int ijk1_, int ijk2_)
        : idx(idx_), cloud_point_index(cloud_point_index_), ijk0(ijk0_), ijk1(ijk1_), ijk2(ijk2_)
      {}

      bool operator<(const CountedPointIndex& other) const
      {
  ...
  VoxelGridCounted::VoxelGridCounted(const float threshold) : threshold_(threshold)
  {
    filter_name_ = "VoxelGridCounted";
  }

  void VoxelGridCounted::setVoxelAlign(const Eigen::Vector4f& align_center)
  {
    align_voxels_ = true;
    align_center_ = align_center;
  }

  void VoxelGridCounted::filter(PointCloudOut& output)
  {
    if (!initCompute ())
    {
      output.points.clear();
  ...
  void VoxelGridCounted::filterImpl(PointCloudOut& output)
  {
    // Has the input dataset been set already?
    if (!input_ || input_->empty() || !indices_ || indices_->empty())
    {
      output.width = output.height = 0;
      output.points.clear ();
      output.is_dense = true;
    ...
    if (!input_->is_dense)
    {
      const bool has_finite_point = std::any_of(indices_->begin(), indices_->end(), [this](const int point_index) {
        const auto& point = input_->points[point_index];
    ...
    std::vector<CountedPointIndex> index_vector;
    ...
    for (int it : *indices_)
    {
      ...
      const CountedPointIndex cpidx(static_cast<unsigned int> (idx), it, ijk0, ijk1, ijk2);
      index_vector.push_back(cpidx);
    }

    // Second pass: sort the index_vector vector using value representing target cell as index
    // in effect all points belonging to the same output cell will be next to each other
    std::sort (index_vector.begin (), index_vector.end (), std::less<CountedPointIndex>());
    ...
    // Fourth pass: insert occupied voxel centers into the returned container together with their count (number of points within over the threshold)
    output.points.resize (total);
    index = 0;
    for (const auto& [first_index, last_index] : first_and_last_indices_vector)
    {
      ...
      {
        const unsigned int cloud_point_index = index_vector[i].cloud_point_index;
        if (input_->points[cloud_point_index].intensity > threshold_)
          ++count;
      }
      output.points[index].x = x;
      output.points[index].y = y;
      output.points[index].z = z;
      output.points[index].range = count;
