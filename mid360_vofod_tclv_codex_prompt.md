...
### 3.3 无返回射线只用于“扫描机会”，不能默认用于自由空间雕刻
...
- 有效返回射线可将传感器到返回点之前的路径更新为空闲；
- 无返回射线可用于判断目标预测区域是否被实际扫描；
- 无返回射线不得直接把传感器到最大量程全部更新为空闲；
- 可额外实现 `oracle_sim_free_carving` 仿真上界模式，但默认关闭，不进入主实验结果。
...
- B0 VoFOD-Mid360 基线只用有效返回射线更新自由空间；
- 不使用无返回射线直接雕刻自由空间；
- B0 不使用无返回方向计算轨迹扫描支持；
- 强化版才使用全部发射方向构建扫描机会层。
